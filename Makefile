...
dev: build
	./bin/imagor -debug -imagor-unsafe -kafka-seed-broker=localhost:9092 -kafka-consume-topic=test-topic -kafka-produce-topic=test-topic
...
docker-dev-run:
	touch .env
	docker run --rm -p 3000:8000 --env-file .env imagor:dev -debug -imagor-unsafe
